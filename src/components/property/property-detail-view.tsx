...
export type PropertyDetailVariant = 'public' | 'app';
...
export function PropertyDetailView({ variant = 'public' }: { variant?: PropertyDetailVariant }) {
  const params = useParams();
  const idStr = params?.id as string;
  const isApp = variant === 'app';
  const propertiesHref = '/app/properties';
  const browseHref = isApp ? '/app' : '/';
  const applyHref = `/property/${idStr}/apply`;
  ...
  if (notFound) {
    return (
      ...
      >
        {!isApp && <PublicNavbar user={user} onLogout={handleLogout} activeLink="browse" />}
        <main
          className={
            isApp
              ? 'flex-1 flex items-center justify-center py-16'
              : 'flex-1 flex items-center justify-center page-container py-16'
          }
        >
          <EmptyState
  ...
  if (initLoading || propertyLoading || !property) {
    return (
      <div
        className={
          isApp
            ? 'flex flex-col bg-background text-foreground font-sans animate-pulse'
            : 'min-h-screen flex flex-col bg-background text-foreground font-sans animate-pulse'
        }
      >
        {!isApp && <PublicNavbarSkeleton />}
        ...
        <main
          className={
            isApp
              ...
              : 'flex-1 max-w-7xl w-full mx-auto px-4 py-8 sm:px-6 lg:px-8 space-y-8'
          }
        >
  ...
  return (
    ...
    >
      ...
      <main className={isApp ? 'flex-1 space-y-6' : 'flex-1 page-container py-6 space-y-6'}>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border/60 pb-4">
          ...
          >
            <Link href={browseHref} className="hover:text-foreground transition-ui">
              {isApp ? 'Home' : 'Browse'}
            </Link>
            <ChevronRight size={12} className="text-muted-foreground/60" aria-hidden />
            <Link href={propertiesHref} className="hover:text-foreground transition-ui">
              ...
              <div className="bg-card border border-border p-5 sm:p-6 rounded-2xl shadow-xs space-y-4">
                <div className="flex items-center justify-between gap-3 border-b border-border/65 pb-2">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-[10px] font-bold text-primary hover:opacity-85 transition-opacity"
                  >
                    Open link
                ...
                {getVideoEmbedUrl(property.videoUrl) ? (
                  <div
                    className={
                      isTikTokVideoUrl(property.videoUrl)
                        ? 'relative mx-auto aspect-[9/16] w-full max-w-[240px] overflow-hidden rounded-xl border border-border bg-muted'
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2.5 rounded-xl border border-border bg-muted/40 px-3 py-2.5 text-xs font-semibold text-foreground hover:bg-muted transition-ui"
                  >
                    <Video size={14} className="text-primary shrink-0" aria-hidden />
